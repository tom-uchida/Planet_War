...
public class Creature2Manager : MonoBehaviour {
	...
	public int creature2Num = 0;

	[Header("Creature2Prefab")]
	public GameObject creature2Prefab;
	...
	void Start () {
		...
		// 配列のインスタンス化
		this.creatures = new GameObject[this.creature2Num];

		// Vector3 tmp = this.plane.transform.position + Random.Range(80f, 120f)*Random.insideUnitSphere;
		// tmp.y = this.plane.transform.position.y;
		Vector3 tmp = this.plane.transform.position + new Vector3( Random.Range(-200, 200), 
																   Random.Range(-10, 10),
																   Random.Range(80, 120) );
		for (int i = 0; i < this.creature2Num; i++) {
			this.creatures[i] = Instantiate(creature2Prefab) as GameObject;
			...
			// 初期位置の設定
			//float scaleParam = Random.Range(0.1f, 2.0f);
		...
		if ( !GameDirector.isGameOver ) {
			...
			foreach (GameObject creature in this.creatures) {
				...
				if ( creature != null ) {
					...
					float diff_Z = this.plane.GetComponent<PlaneController>().planeSpeed*15.0f 
						 + 40*Mathf.Sin(1.0f * Time.time);

					float diff_X = 100*Mathf.Sin(1f * Time.time);
